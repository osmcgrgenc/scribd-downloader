...
class EverandDownloader {
    ...
    async execute(url, reporter = cliReporter) {
        if (url.match(everandRegex.PODCAST_SERIES)) {
            await this.series(url, reporter)
        } else if (url.match(everandRegex.PODCAST_EPISODE)) {
            await this.listen(`https://www.everand.com/listen/podcast/${everandRegex.PODCAST_EPISODE.exec(url)[1]}`, true, reporter)
        } else if (url.match(everandRegex.PODCAST_LISTEN)) {
            await this.listen(url, true, reporter)
        } else {
            throw new Error(`Unsupported URL: ${url}`)
        }
    }

    async listen(url, isEpisode, reporter) {
    ...
    async series(url, reporter) {
        ...
        reporter.log("Downloading series...")
        ...
        for (let i = 1; i <= totalPage; i++) {
            ...
            for (let j = 0; j < episodes.length; j++ ) {
                await this.listen(episodes[j], false, reporter)
                seriesProgress.update(((i - 1) * 10) + (j + 1))
            }
        }
        seriesProgress.stop()
