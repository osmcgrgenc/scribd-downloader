...
class ScribdDownloader {
    ...
    async execute(url, flag, reporter = cliReporter) {
        ...
        if (url.match(scribdRegex.DOCUMENT)) {
            await fn(`https://www.scribd.com/embeds/${scribdRegex.DOCUMENT.exec(url)[2]}/content`, reporter)
        } else if (url.match(scribdRegex.EMBED)) {
            await fn(url, reporter)
        } else {
            throw new Error(`Unsupported URL: ${url}`)
        }
    }

    async embedsDefault(url, reporter) {
        ...
        if (m) {
            ...
            reporter.log("Generate per-page PDFs...")
            ...
            pdfProgress.update(pageSelectors.length);
            pdfProgress.stop();
            ...
            for (let i = 0; i < pageSelectors.length; i++) {
                ...
                try {
                    ...
                    reporter.error(`Failed to merge PDF at ${tmpPdfPath}: ${error.message}`)
                }
            }
            ...
            reporter.log(`Generated: ${outputPdfPath}`)

            // remove temporary directory
            try {
                await fs.rm(`${output}/${identifier}`, { recursive: true, force: true });
            } catch (error) {
                reporter.error(`Failed to delete temporary directory at ${output}/${identifier}: ${error.message}`)
            }
    ...
    async embedsImage(url, reporter) {
